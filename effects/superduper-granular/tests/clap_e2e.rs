//! End-to-end CLAP integration test for SuperDuper Granular.
//!
...
//!
//! Run: `cargo test --release -p superduper-granular --test clap_e2e -- --nocapture`
...
use superduper_granular::SuperDuperGranular;
...
#[test]
fn granular_clap_pipeline_produces_audio() {
    let entry = PluginEntry::load_from_clack::<SinglePluginEntry<SuperDuperGranular>>(
        c"/in/process/test/superduper-granular",
    )
    ...
    assert_eq!(desc.id().unwrap().to_bytes(), b"co.superduperai.granular");
