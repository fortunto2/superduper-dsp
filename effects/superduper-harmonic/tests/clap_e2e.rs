//! End-to-end CLAP integration test for SuperDuper Harmonic.
//!
//! Loads the plugin through `clack-host` (no REAPER), activates it, streams a
//! harmonic drone + noise through the main stereo input, and asserts the output
//! isn't silent and is finite. Catches CLAP-plumbing bugs (audio-ports, param
//! routing) that `dsp_smoke` can't.
//!
//! Run: `cargo test -p superduper-harmonic --test clap_e2e -- --nocapture`
...
use superduper_harmonic::SuperDuperHarmonic;
...
#[test]
fn harmonic_clap_pipeline_produces_audio() {
    let entry = PluginEntry::load_from_clack::<SinglePluginEntry<SuperDuperHarmonic>>(
        c"/in/process/test/superduper-harmonic",
    )
    ...
    assert_eq!(desc.id().unwrap().to_bytes(), b"co.superduperai.harmonic");
    ...
    // 2 s of a 90 Hz harmonic drone + low broadband noise.
    const TOTAL_FRAMES: usize = (SR as usize) * 2;
    ...
    for i in 0..TOTAL_FRAMES {
        ...
        let mut v = 0.0;
        for h in 1..=10 {
            v += (0.5 / h as f32) * (std::f32::consts::TAU * 90.0 * h as f32 * t).sin();
        }
        s ^= s << 13;
        s ^= s >> 17;
        s ^= s << 5;
        let noise = (s as f32 / u32::MAX as f32) * 2.0 - 1.0;
        let x = v * 0.25 + noise * 0.03;
        all_in_l[i] = x;
        all_in_r[i] = x;
    }
    ...
    let stopped_back = std::thread::scope(|sc| {
        sc.spawn(move || {
    ...
    let tail_l = &all_out_l[TOTAL_FRAMES / 2..];
    ...
    assert!(
        peak_l > 1e-3,
        "harmonic output is silent through the CLAP pipeline (peak={peak_l}) — \
