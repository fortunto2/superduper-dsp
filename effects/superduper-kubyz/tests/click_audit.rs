//! click_audit.rs — drive Kubyz through a voice-steal + same-key-retrigger
//! sequence and measure sample-to-sample discontinuities (clicks).
//!
//! Same class of bug the user hit on Wave: playing fast over a sustained drone
//! makes new notes (a) steal busy voices and (b) retrigger held keys. The old
//! code hard-swapped stolen voices and routed retrigger through `gate_on`
//! (PreDelay zeroes the level → a drop-to-zero click). This asserts those are
//! gone. Writes /tmp/kubyz_click_audit.wav for listening.
//!
//! Run: cargo test --release -p superduper-kubyz --test click_audit -- --nocapture
//! Listen: afplay /tmp/kubyz_click_audit.wav
...
// Drone patch: hold voices at a high sustain + long release so the pool stays
// busy and stolen/retriggered voices are at full amplitude.
const P_SUSTAIN: u32 = 8;
const P_RELEASE: u32 = 9;
...
fn build_sequence() -> Vec<(usize, MidiAt)> {
    ...
    let mut seq = Vec::new();
    // 8-note drone cluster fills the pool.
    let drone: [u8; 8] = [36, 40, 43, 48, 52, 55, 60, 64];
    for (i, k) in drone.iter().enumerate() {
        seq.push((t(0.10 + i as f32 * 0.08), MidiAt::On(*k, 0.85)));
    }
    // Fast stabs on top — some keys overlap the drone (60, 64) to force the
    // same-key retrigger path; the rest force steals.
    let stab_start = 2.0;
    let stab_keys: [u8; 16] = [
...
fn render(params: &[(u32, f64)]) -> (Vec<f32>, Vec<f32>) {
    let entry = PluginEntry::load_from_clack::<SinglePluginEntry<SuperDuperKubyz>>(
        c"/in/process/test/superduper-kubyz-click",
    )
    .expect("entry");
    let host_info = HostInfo::new("t", "t", "t", "0").unwrap();
    let mut plugin =
        PluginInstance::<TH>::new(|_| TS, |_| (), &entry, c"co.superduperai.kubyz", &host_info)
            .expect("instantiate");
...
/// Anomaly detector: |Δ[i]| / local-median-|Δ|. A click is many× the local
/// slope; smooth zero-crossings of the voice sum sit around 6-7×.
...
#[test]
fn kubyz_click_audit_steal_retrigger() {
    let (l, r) = render(&[(P_SUSTAIN, 0.85), (P_RELEASE, 1.5)]);
    write_wav("/tmp/kubyz_click_audit.wav", &l, &r);
    let (mj, at, anom) = analyse(&l);
    eprintln!(
        "[kubyz] max raw jump = {:.4} at t={:.3}s  → /tmp/kubyz_click_audit.wav",
        mj,
        at as f32 / SR
    );
    ...
    // Deferred steal + legato retrigger keep note-event seams smooth; a
    // regression to hard-swap / gate_on retrigger brings back 15-30×.
    assert!(
        worst < 12.0,
        "kubyz voice click regressed (hard swap / gate_on retrigger?): {worst:.1}× (want < 12)"
    );
}
