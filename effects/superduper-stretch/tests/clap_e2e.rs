//! End-to-end CLAP integration test for SuperDuper Stretch.
//!
...
//!
//! Run: `cargo test --release -p superduper-stretch --test clap_e2e -- --nocapture`
...
use superduper_stretch::SuperDuperStretch;
...
#[test]
fn stretch_clap_pipeline_produces_audio() {
    let entry = PluginEntry::load_from_clack::<SinglePluginEntry<SuperDuperStretch>>(
        c"/in/process/test/superduper-stretch",
    )
    ...
    assert_eq!(desc.id().unwrap().to_bytes(), b"co.superduperai.stretch");
