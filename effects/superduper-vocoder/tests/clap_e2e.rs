//! End-to-end CLAP integration test for SuperDuper Vocoder.
//!
//! Loads the plugin through `clack-host` (no REAPER), activates it, and
//! streams a voiced tone through the main input with the default internal
//! carrier — then asserts the output isn't silent. Catches CLAP-plumbing
//! bugs (audio-ports declaration, process routing) that `dsp_smoke` can't.
//!
//! Run: `cargo test -p superduper-vocoder --test clap_e2e -- --nocapture`
...
use superduper_vocoder::SuperDuperVocoder;
...
#[test]
fn vocoder_clap_pipeline_produces_audio() {
    let entry = PluginEntry::load_from_clack::<SinglePluginEntry<SuperDuperVocoder>>(
        c"/in/process/test/superduper-vocoder",
    )
    ...
    assert_eq!(desc.id().unwrap().to_bytes(), b"co.superduperai.vocoder");
    ...
    // 2 s of a voiced 220 Hz tone (with harmonics) as the modulator.
    ...
    // Analyse the second half (past the pitch-tracker warm-up).
    ...
    assert!(
        peak_l > 1e-3,
        "vocoder output is silent through the CLAP pipeline (peak={peak_l}) — \
